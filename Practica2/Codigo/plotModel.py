...
def plotModel(x,y,clase,clf,title, dataset):
    ...
    aux = np.c_[xx.ravel(), yy.ravel(), zeros]
    indices = np.arange(len(aux) - 1)
    clf.entrenamiento(dataset,  indices)

    if isinstance(clf, Clasificador.Clasificador):	

        pred = clf.clasifica(np.c_[xx.ravel(), yy.ravel(), zeros],indices)
        z = np.array(pred)
